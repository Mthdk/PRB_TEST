...
# Função para contar imagens em uma pasta
...
with open('relatorio.csv', 'w', newline='') as arquivo_csv:
    ...
    escritor_csv.writerow(['Pasta', 'Quantidade de Imagens'])
    ...
    for pasta_raiz, subpastas, arquivos in os.walk(diretorio_base):
        ...
        if any(is_imagem(arquivo) for arquivo in arquivos):
            quantidade_imagens = contar_imagens_em_pasta(pasta_raiz)
            escritor_csv.writerow([pasta_raiz, quantidade_imagens])
